...
const EditPredictionModal = ({ isOpen, onClose, prediction, onUpdate }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        setIsSaving(true);
        axios.put(`${process.env.REACT_APP_API_URL}/api/predictions/${prediction._id}/edit`, {
    ...
    const percentageChange = ((parseFloat(target) - prediction.targetPrice) / prediction.targetPrice) * 100;
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 bg-gray-900 rounded-md pr-2">
                                ...
                                <span className={`font-bold text-sm ${!isNaN(percentageChange) && percentageChange >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                    {!isNaN(percentageChange) ? formatPercentage(percentageChange, i18n.language) : ''}
                                </span>
                            </div>
                        </div>
