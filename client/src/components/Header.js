...
const Header = ({ user, onMakePredictionClick, settings }) => {
    ...
    const getLoginUrl = () => {
        ...
        let url = `${process.env.REACT_APP_API_URL}/auth/google?redirect=/`;
        if (refCode) {
            url += `&ref=${refCode}`;
        }
        return url;
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center space-x-4 2xl:space-x-8">
                            ...
                            <div className="hidden 2xl:flex items-center space-x-6">
                                ...
                                {/* --- LINK UPDATED --- */}
                                {/* --- FIX: Link is now visible to everyone --- */}
                    ...
                    {isMobileMenuOpen && (
                        <div className="2xl:hidden mt-4 space-y-1">
                            ...
                            <div className="mt-2">
                                {user ? <a href={`${process.env.REACT_APP_API_URL}/auth/logout`} className="block w-full text-center py-2 px-4 text-sm bg-red-600 rounded">{t('header.mobileMenu.logout')}</a> : <a href={getLoginUrl()} className="block w-full text-center py-2 px-4 text-sm bg-blue-600 rounded text-center">{t('header.mobileMenu.login')}</a>}
                            </div>
                        </div>
                    )}
                </div>
