...
const PredictionJourney = ({ initial, target, current, priceLabel }) => {
  ...
  const DashedLine = () => (
    <div
      className="h-2 bg-gray-700/50 rounded-full w-full"
      style={{
        backgroundImage:
          'repeating-linear-gradient(-45deg, transparent, transparent 4px, #4a5568 4px, #4a5568 5px)',
  ...
  const CurrentPriceMarker = () => (
    <div
      ...
      style={{
        left: zone === 'middle' ? `${positionInZone}%` : '50%',
    ...
    >
      <div className="relative">
        <button
          onClick={() =>
            setVisibleTooltip(visibleTooltip === 'current' ? null : 'current')
          }
    ...
    <div className="my-12">
      <div className="relative w-full flex items-center">
        ...
        <div className="relative flex-[2_2_0%] mx-2 h-2 bg-green-500 rounded-full">
          ...
          <div
            className="absolute top-1/2 -translate-y-1/2"
            style={{ left: isUpward ? '0%' : '100%', transform: 'translateX(-50%)' }}
          >
            <div className="relative flex flex-col items-center">
              <button
                onClick={() =>
                  setVisibleTooltip(visibleTooltip === 'initial' ? null : 'initial')
                }
                className="relative z-10"
              >
          ...
          <div
            className="absolute top-1/2 -translate-y-1/2"
            style={{ left: isUpward ? '100%' : '0%', transform: 'translateX(-50%)' }}
          >
            <div className="relative flex flex-col items-center">
              <button
                onClick={() =>
                  setVisibleTooltip(visibleTooltip === 'target' ? null : 'target')
                }
                className="relative z-10"
              >
                <div
