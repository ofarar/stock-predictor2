...
const MiniPredictionCard = ({ prediction, currentPrice, isOwnProfile, onEditClick }) => {
    ...
    return (
        <div className="relative">
            <Link to={`/prediction/${prediction._id}`} className="block bg-gray-700 p-3 rounded-lg hover:bg-gray-600 transition-colors pe-8">
            ...
            {isOwnProfile && prediction.status === 'Active' && (
                <button
                    ...
                    className="absolute top-1/2 end-1 -translate-y-1/2 p-1 bg-gray-800 rounded-full text-gray-400 hover:text-white"
