    ...
    return (
        <div className="bg-gray-800 p-6 rounded-xl">
            <div className="flex items-center mb-4">
                ...
                <h3 className="text-xl font-bold text-white">Trending Predictions</h3>
            </div>
            <div className="space-y-3">
                {stocks.length > 0 ? stocks.map(stock => (
                    <Link to={`/stock/${stock.ticker}`} key={stock.ticker} className="flex justify-between items-center bg-gray-700 p-3 rounded-lg hover:bg-gray-600 transition-colors">
                        <span className="font-bold text-white">{stock.ticker}</span>
                        <span className="text-sm text-gray-400">{stock.predictions} predictions today</span>
                    </Link>
                )) : (
                    <p className="text-gray-500 text-center py-4">No trending stocks right now.</p>
                )}
            </div>
        </div>
    );
