...
const BadgeShowcase = ({ badges = [], onBadgeClick, onInfoClick }) => {
    ...
    const renderContent = () => {
        if (badges.length === 0) return <p className="text-gray-500 px-6 text-center sm:text-left">No achievements yet. Keep making predictions!</p>;
        ...
        return (
            <div className="flex gap-4 overflow-x-auto pb-4 px-6 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
                {sortedBadges.map((badge, index) => {
                    if (!badgeDetails) return null; // Don't render badges until details are loaded
                    const details = badgeDetails[badge.badgeId] || { name: 'Unknown Badge' };
    ...
    return (
        <div className="bg-gray-800 rounded-lg py-6">
            <div className="flex items-center justify-between px-6 mb-4">
                ...
                <button onClick={onInfoClick} className="text-gray-400 hover:text-white" title="How are badges earned?">
