...
const StatCard = ({ label, avgScore, rank, isStock, aggressivenessScore }) => {
    const { t } = useTranslation();
    const circumference = 2 * Math.PI * 20;
    const offset = circumference - (avgScore / 100) * circumference;

    return (
        <div className="flex flex-col bg-gray-700 p-4 rounded-lg transition-transform hover:scale-[1.02]">
    ...
    return (
        <div className="bg-gray-800 p-4 sm:p-6 rounded-lg">
            <div className="flex border-b border-gray-700 mb-4">
                <button onClick={() => setActiveTab('ByType')} className={`px-4 py-2 font-bold transition-colors ${activeTab === 'ByType' ? 'text-green-400 border-b-2 border-green-400' : 'text-gray-400 hover:text-white'}`}>
                    {t('performanceTabs.tabs.byType')}
                </button>
                <button onClick={() => setActiveTab('ByStock')} className={`px-4 py-2 font-bold transition-colors ${activeTab === 'ByStock' ? 'text-green-400 border-b-2 border-green-400' : 'text-gray-400 hover:text-white'}`}>
            ...
            {activeTab === 'ByType' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in-fast">
                    {performance.byType?.length > 0 ? performance.byType.map(p => (
                        <StatCard key={p.type} label={t(`predictionTypes.${p.type.toLowerCase()}`)} avgScore={p.accuracy} rank={p.rank} aggressivenessScore={p.aggressivenessScore} />
            ...
            {activeTab === 'ByStock' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in-fast">
                    {performance.byStock?.length > 0 ? performance.byStock.map(s => (
                        <StatCard key={s.ticker} label={s.ticker} avgScore={s.accuracy} rank={s.rank} isStock={true} aggressivenessScore={s.aggressivenessScore} />
