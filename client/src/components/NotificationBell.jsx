...
const getNotificationIcon = (type) => {
    if (type === 'GoldenPost') {
        return <svg className="w-5 h-5 me-3 text-yellow-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>;
    }
    switch (type) {
        case 'NewFollower':
            return <svg className="w-5 h-5 me-3 text-blue-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>;
        case 'NewReferral': // <-- Make sure this is in your Notification.js model
            return <svg className="w-5 h-5 me-3 text-green-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path></svg>;
        case 'PriceChange':
            return <svg className="w-5 h-5 me-3 text-blue-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0c-1.657 0-3-.895-3-2s1.343-2 3-2 3-.895 3-2-1.343-2-3-2m0 8c1.11 0 2.08-.402 2.599-1M12 16v1m0-1v-8"></path></svg>;
        case 'BadgeEarned':
            return <svg className="w-5 h-5 me-3 text-yellow-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"></path></svg>;
        case 'PredictionAssessed':
        default: // NewPrediction
            return <svg className="w-5 h-5 me-3 text-green-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>;
    }
}
...
const NotificationBell = ({ user }) => {
    ...
    useEffect(() => {
        ...
        if (prevIsOpen.current === true && isOpen === false && unreadCount > 0) {
            ...
            axios.post(`${import.meta.env.VITE_API_URL}/api/notifications/mark-read`, {}, { withCredentials: true })
                .then(() => {
    ...
    const handleClearNotifications = () => {
        const promise = axios.delete(`${import.meta.env.VITE_API_URL}/api/notifications/clear`, { withCredentials: true });
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button onClick={handleBellClick} className="relative p-2">
                ...
                {unreadCount > 0 && (
                    <span className={`absolute top-1 end-1 block h-2.5 w-2.5 rounded-full border-2 border-gray-900 ${hasUnreadGoldenPost ? 'bg-yellow-400' : 'bg-red-500'}`}></span>
                )}
            </button>

            {isOpen && (
                <div className="absolute end-0 mt-2 w-80 bg-gray-800 rounded-lg shadow-xl flex flex-col z-40">
                    ...
                    <div className="max-h-96 overflow-y-auto">
                        {notifications.length > 0 ? notifications.map(n => {
                            ...
                            const messageKey = n.messageKey || 'notifications.default';
                            ...
                            return (
                                <Link to={n.link} key={n._id} onClick={() => setIsOpen(false)} className="flex items-start p-2 text-sm text-gray-300 hover:bg-gray-700 rounded">
                                    ...
                                    <div className="flex-grow">
                                        <span className={!n.read ? 'font-bold' : ''}>{t(messageKey, interpolation)}</span>
                                        {n.metadata?.percentage != null && (
                                            <span className={`ms-1 font-bold ${n.metadata.percentage >= 0 ? 'text-green-400' : 'text-red-400'}`}>
