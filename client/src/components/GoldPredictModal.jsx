...
const GoldPredictModal = ({ isOpen, onClose, onPredictionSuccess }) => {
    ...
    const handleRunAnalysis = async () => {
        if (!ticker) return toast.error(t('instantPredict.inputLabel') + " required"); // Fallback or add error key
        ...
        try {
            ...
            toast.success(t('instantPredict.analysisComplete'));

            if (onPredictionSuccess) onPredictionSuccess(res.data);
            // onClose(); // formatted: Don't close, show result!
        } catch (error) {
            // toast.dismiss(toastId);
            toast.error(error.response?.data?.message || t('instantPredict.analysisFailed'));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const renderResultView = () => {
        ...
        const { ticker: resultTicker, current_price, prediction, target_price, pct_move, timeframe, rationale, interval } = predictionResult;
            ...
            <div className="flex flex-col items-center animate-fade-in">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                            ...
                            <span className="text-sm font-normal text-gray-400 bg-gray-700 px-2 py-0.5 rounded-full border border-gray-600">
                                {interval === '1d' ? t('instantPredict.labelDaily') : t('instantPredict.labelIntraday')}
                            </span>
                        </h2>
                        <div className="text-gray-400 text-sm mt-1">{t('instantPredict.currentPriceLabel')}: <span className="text-white font-mono">${current_price?.toFixed(2)}</span></div>
                    </div>
                    {/* Timeframe Badge */}
                    <div className="flex flex-col items-end">
                        <span className="text-[10px] text-gray-400 uppercase tracking-wider mb-1">{t('instantPredict.horizonLabel')}</span>
                ...
                <div className={`w-full p-6 rounded-xl border mb-6 flex flex-col items-center text-center shadow-2xl relative overflow-hidden ${isBullish ? 'bg-green-900/20 border-green-500/50' : 'bg-red-900/20 border-red-500/50'}`}>
                    ...
                    <div className="flex items-baseline gap-2 mb-4">
                        <span className="text-gray-300 text-lg">{t('instantPredict.targetLabel')}:</span>
                    ...
                    <span className={`px-3 py-1 rounded text-sm font-bold border ${isBullish ? 'bg-green-900/40 text-green-300 border-green-500/30' : 'bg-red-900/40 text-red-300 border-red-500/30'}`}>
                        {t('instantPredict.expectedMoveLabel')}: {pct_move > 0 ? '+' : ''}{(pct_move * 100).toFixed(2)}%
                    </span>
                </div>

                <div className="w-full bg-gray-900/50 p-4 rounded border border-gray-700 mb-6">
                    <span className="text-xs text-gray-500 uppercase font-bold block mb-1">{t('instantPredict.rationaleLabel')}</span>
                ...
                >
                    {t('instantPredict.closeResultButton')}
                </button>
            </div>
        );
    };
                ...
                {!predictionResult ? (
                    <>
                        <div className="flex flex-col items-center mb-6">
                            ...
                            <h2 className="text-2xl font-bold text-white">{t('instantPredict.modalTitle')}</h2>
                            <p className="text-gray-400 text-sm mt-1 text-center">
                                {t('instantPredict.modalSubtitle')}
                            </p>
                        </div>

                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-300 mb-1">{t('instantPredict.inputLabel')}</label>
                                <TickerAutocomplete
                                    value={ticker}
                                    onChange={setTicker}
                                    onSelect={(val) => setTicker(val)}
                                    placeholder={t('instantPredict.inputPlaceholder')}
                                    className="bg-gray-900 border-gray-700"
                                />
                                <p className="text-[10px] text-gray-500 mt-1 text-right">{t('instantPredict.disclaimer')}</p>
                            </div>
                            ...
                            >
                                {isLoading ? (
                                    <>
                                        ...
                                        {t('instantPredict.analyzing')}
                                    </>
                                ) : (
                                    t('instantPredict.runButton')
                                )}
                            </button>
                        </div>
                    </>
