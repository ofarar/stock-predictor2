...
const isMarketOpen = () => {
    ...
    // The market is open if it's a weekday AND after the open time AND before the close time.
    return isWeekday && isAfterOpen && isBeforeClose;
};

// This function now contains all the time and penalty logic
const getPredictionDetails = (predictionType) => {
    ...
    // Check market status for short-term predictions
    if (!isMarketOpen() && (predictionType === 'Hourly' || predictionType === 'Daily')) {
        return { isOpen: false, message: 'Market is currently closed', barWidth: '0%' };
    }
    ...
    switch (predictionType) {
        ...
        case 'Daily': {
            ...
            deadline.setUTCHours(20, 0, 0, 0); // 4 PM ET
            break;
        }
        case 'Weekly': {
            ...
            maxScore = 100 - penalty;
            barWidth = 100 - ((dayOfWeek - 1) / 4 * 100);
            break;
        }
        case 'Monthly': {
...
const PredictionWidget = ({ onClose, initialStock }) => {
    ...
    useEffect(() => {
        ...
        // Only run a frequent timer for Hourly/Daily predictions
