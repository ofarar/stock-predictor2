...
const GoldenMemberModal = ({ isOpen, onClose, user, onUpdate }) => {
    ...
    const handleSaveSettings = async (willBeGolden) => {
        ...
        try {
            ...
            await axios.put(`${process.env.REACT_APP_API_URL}/api/profile/golden-member`, settingsPayload, { withCredentials: true });
    ...
    const handleStartOnboarding = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/stripe/connect/onboarding-link`, {}, { withCredentials: true });
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="relative bg-gray-800 p-8 rounded-xl shadow-2xl w-full max-w-lg text-gray-300" onClick={e => e.stopPropagation()}>
                <button onClick={onClose} disabled={isSaving} className="absolute top-4 right-4 text-gray-400 hover:text-white disabled:opacity-50">
