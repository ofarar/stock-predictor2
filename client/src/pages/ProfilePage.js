...
// Helper component for the performance chart with responsive filters
...
// Helper component for the performance tabs, now fully responsive
            ...
            {activeTab === 'ByType' && (
                <div className="space-y-2">
                    {performance.byType?.length > 0 ? performance.byType.map(p => (
                        <div key={p.type} className="flex flex-col sm:flex-row sm:justify-between text-sm p-2 bg-gray-700 rounded">
                            <span className="font-bold text-gray-300">{p.type}</span>
                            <div className="text-gray-400"><span className="font-semibold text-gray-300">{p.accuracy}</span> Acc | <span className="font-semibold text-gray-300">{p.rank}</span> Rank</div>
                        </div>
...
const ProfilePage = () => {
    ...
    const handleFollow = () => {
        axios.post(`${process.env.REACT_APP_API_URL}/api/users/${userId}/follow`, {}, { withCredentials: true })
            .then(() => {
                ...
                setCurrentUser(prevUser => ({ ...prevUser, following: [...prevUser.following, userId] }));
                setProfileData(prevData => ({ ...prevData, followersCount: prevData.followersCount + 1 }));
            });
    };
    ...
    const handleUnfollow = () => {
        axios.post(`${process.env.REACT_APP_API_URL}/api/users/${userId}/unfollow`, {}, { withCredentials: true })
            .then(() => {
                // Update state instantly for better UX
                setCurrentUser(prevUser => ({ ...prevUser, following: prevUser.following.filter(id => id !== userId) }));
                setProfileData(prevData => ({ ...prevData, followersCount: prevData.followersCount - 1 }));
            });
    };
    ...
    return (
        <div className="animate-fade-in max-w-6xl mx-auto">
            <div className="flex flex-col sm:flex-row items-center gap-6 bg-gray-800 p-6 rounded-lg mb-8">
                <img src={user.avatar || `https://avatar.iran.liara.run/public/boy?username=${user._id}`} alt="avatar" className="w-24 h-24 rounded-full border-4 border-green-500" />
                <div className="flex-grow text-center sm:text-left">
                    ...
                    <div className="mt-4 flex items-center justify-center sm:justify-start space-x-4">
                        <Link to={`/profile/${userId}/followers`} className="text-sm text-gray-400 hover:underline"><span className="font-bold text-white">{followersCount || 0}</span> Followers</Link>
                        <Link to={`/profile/${userId}/followers`} state={{ activeTab: 'Following' }} className="text-sm text-gray-400 hover:underline"><span className="font-bold text-white">{followingCount || 0}</span> Following</Link>
                    </div>
                </div>
                <div className="ml-auto mt-4 sm:mt-0">
                    {currentUser && !isOwnProfile && (
                        isFollowing ? (
                            <button onClick={handleUnfollow} className="bg-gray-700 text-white font-bold py-2 px-4 rounded-md hover:bg-red-600">Following</button>
                        ) : (
